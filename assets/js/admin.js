	...
	var Frozr_Admin_Settings = {
		...
		frozr_edit_fee: function() {
			var r_div = $(this).data('rule'),
				...
				$('#'+r_div).show();
				$('.frozr_fee_settings .customers_effected select, .frozr_fee_settings .sellers_effected select').each(function() {
					$(this).trigger('change');
				});
		...
		frozr_rule_duplicate: function(e) {
			...
			$.ajax({
				url: norsani_admin_script_params.ajax_url,
				data: {
		...
		reorder_seller_fee_rules: function () {
			...
			$('.frozr_seller_fee_rule', wrapper).each(function() {
				$('.form-group', this).each(function() {
					var rule = $(this),
						tdclass = rule.find('label').attr('for');
						tds_new_classes = tdclass.replace(/\d+/,counter).replace(/new/g, counter);

					rule.removeClass().addClass('form-group ' + tds_new_classes + '_field');
					rule.find('label').attr('for', tds_new_classes);
					rule.find('input').attr({"id": tds_new_classes, "name": tds_new_classes});
					rule.find('select:not(.multiopt)').attr({"id": tds_new_classes, "name": tds_new_classes});
					rule.find('.multiopt').attr({"id": tds_new_classes, "name": tds_new_classes+'[]'});
				});
				counter++;
			});
